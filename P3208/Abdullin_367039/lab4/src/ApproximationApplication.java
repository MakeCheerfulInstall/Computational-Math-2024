...
import Abdullin_367039.lab4.LinearApproximation;
import Abdullin_367039.lab4.PowerApproximation;
import Abdullin_367039.lab4.QuadraticApproximation;
import Abdullin_367039.lab4.QubicApproximation;
...
public class ApproximationApplication {
  public static void main(String[] args) {
    ...
    if (inputChoice == 1) {
      ...
      try {
        FileReader fr = new FileReader("/home/yestai/IdeaProjects/Computational-Math-2024/P3208/src/Abdullin_367039/lab4/resources/test1");
    ...
    Abdullin_367039.lab4.LinearApproximation linearApproximation = new LinearApproximation();
    Abdullin_367039.lab4.QuadraticApproximation quadraticApproximation = new QuadraticApproximation();
    Abdullin_367039.lab4.QubicApproximation qubicApproximation = new QubicApproximation();
    ...
    Abdullin_367039.lab4.PowerApproximation powerApproximation = new PowerApproximation();
    ...
    if (outputChoice == 1) {
      ...
      System.out.println(
          "Линейная аппроксимация: a = "
              + linearApproximation.getA()
              + ", b = "
              + linearApproximation.getB());
      for (int i = 0; i < amount; i++) {
        System.out.println(
            "x = "
                + x[i]
                ...
                + y[i]
                + ", phi(x) = "
                + linearApproximation.solve(x, y, amount)[i]
                + ", eps = "
                + linearApproximation.getEpsilon()[i]);
      }
      System.out.println(
          "Коэффициент корреляции Пирсона: "
              + linearApproximation.getPearsonCoefficient(x, y, amount));
      System.out.println(
          "Коэффициент детерминации: "
              + linearApproximation.getDeterminationCoefficient(
                  linearApproximation.solve(x, y, amount), y, amount));
      if (linearApproximation.getDeterminationCoefficient(
              linearApproximation.solve(x, y, amount), y, amount)
          >= 0.95) {
        System.out.println("Высокая точность аппроксимации");
      } else if (linearApproximation.getDeterminationCoefficient(
              linearApproximation.solve(x, y, amount), y, amount)
          >= 0.75) {
      ...
      System.out.println(
          "Квадратичная аппроксимация: a0 = "
              + quadraticApproximation.getA0()
              + ", a1 = "
              + quadraticApproximation.getA2()
              + ", a2 = "
              + quadraticApproximation.getA2());
      for (int i = 0; i < amount; i++) {
        System.out.println(
            "x = "
                ...
                + y[i]
                + ", phi(x) = "
                + quadraticApproximation.solve(x, y, amount)[i]
                + ", eps = "
                + quadraticApproximation.getEpsilon()[i]);
      }
      System.out.println(
          "Коэффициент детерминации: "
              + quadraticApproximation.getDeterminationCoefficient(
                  quadraticApproximation.solve(x, y, amount), y, amount));
      if (quadraticApproximation.getDeterminationCoefficient(
              quadraticApproximation.solve(x, y, amount), y, amount)
          >= 0.95) {
        System.out.println("Высокая точность аппроксимации");
      } else if (quadraticApproximation.getDeterminationCoefficient(
              quadraticApproximation.solve(x, y, amount), y, amount)
          >= 0.75) {
        System.out.println("Удовлетворительная аппроксимация");
      } else if (quadraticApproximation.getDeterminationCoefficient(
      ...
      System.out.println(
          "Кубическая аппроксимация: a0 = "
              + qubicApproximation.getA0()
              + ", a1 = "
              + qubicApproximation.getA1()
              + ", a2 = "
              + qubicApproximation.getA2()
              + ", a3 = "
              + qubicApproximation.getA3());
      for (int i = 0; i < amount; i++) {
        System.out.println(
            "x = "
                + x[i]
                ...
                + y[i]
                + ", phi(x) = "
                + qubicApproximation.solve(x, y, amount)[i]
                + ", eps = "
                + qubicApproximation.getEpsilon()[i]);
      }
      System.out.println(
          "Коэффициент детерминации: "
              + qubicApproximation.getDeterminationCoefficient(
                  qubicApproximation.solve(x, y, amount), y, amount));
      if (qubicApproximation.getDeterminationCoefficient(
              qubicApproximation.solve(x, y, amount), y, amount)
          >= 0.95) {
        System.out.println("Высокая точность аппроксимации");
      } else if (qubicApproximation.getDeterminationCoefficient(
              qubicApproximation.solve(x, y, amount), y, amount)
          >= 0.75) {
        System.out.println("Удовлетворительная аппроксимация");
      } else if (qubicApproximation.getDeterminationCoefficient(
      ...
      System.out.println(
          "Экспоненциальная аппроксимация: a = "
              + exponentialApproximation.getA()
              + ", b = "
              + exponentialApproximation.getB());
      for (int i = 0; i < amount; i++) {
        System.out.println(
            "x = "
                + x[i]
                ...
                + y[i]
                + ", phi(x) = "
                + exponentialApproximation.solve(x, y, amount)[i]
                + ", eps = "
                + exponentialApproximation.getEpsilon()[i]);
      }
      System.out.println(
          "Коэффициент детерминации: "
              + exponentialApproximation.getDeterminationCoefficient(
                  exponentialApproximation.solve(x, y, amount), y, amount));
      if (exponentialApproximation.getDeterminationCoefficient(
              exponentialApproximation.solve(x, y, amount), y, amount)
          >= 0.95) {
        System.out.println("Высокая точность аппроксимации");
      } else if (exponentialApproximation.getDeterminationCoefficient(
              exponentialApproximation.solve(x, y, amount), y, amount)
          >= 0.75) {
        System.out.println("Удовлетворительная аппроксимация");
      } else if (exponentialApproximation.getDeterminationCoefficient(
      ...
      System.out.println(
          "Логарифмическая аппроксимация: a = "
              + logarithmicApproximation.getA()
              + ", b = "
              + logarithmicApproximation.getB());
      for (int i = 0; i < amount; i++) {
        System.out.println(
            "x = "
                + x[i]
                ...
                + y[i]
                + ", phi(x) = "
                + logarithmicApproximation.solve(x, y, amount)[i]
                + ", eps = "
                + logarithmicApproximation.getEpsilon()[i]);
      }
      System.out.println(
          "Коэффициент детерминации: "
              + logarithmicApproximation.getDeterminationCoefficient(
                  logarithmicApproximation.solve(x, y, amount), y, amount));
      if (logarithmicApproximation.getDeterminationCoefficient(
              logarithmicApproximation.solve(x, y, amount), y, amount)
          >= 0.95) {
        System.out.println("Высокая точность аппроксимации");
      } else if (logarithmicApproximation.getDeterminationCoefficient(
              logarithmicApproximation.solve(x, y, amount), y, amount)
          >= 0.75) {
        System.out.println("Удовлетворительная аппроксимация");
      } else if (logarithmicApproximation.getDeterminationCoefficient(
      ...
      for (int i = 0; i < amount; i++) {
        System.out.println(
            "x = "
                ...
                + y[i]
                + ", phi(x) = "
                + powerApproximation.solve(x, y, amount)[i]
                + ", eps = "
                + powerApproximation.getEpsilon()[i]);
      }
      System.out.println(
          "Коэффициент детерминации: "
              + powerApproximation.getDeterminationCoefficient(
                  powerApproximation.solve(x, y, amount), y, amount));
      if (powerApproximation.getDeterminationCoefficient(
              powerApproximation.solve(x, y, amount), y, amount)
          >= 0.95) {
        System.out.println("Высокая точность аппроксимации");
      } else if (powerApproximation.getDeterminationCoefficient(
              powerApproximation.solve(x, y, amount), y, amount)
          >= 0.75) {
        System.out.println("Удовлетворительная аппроксимация");
      } else if (powerApproximation.getDeterminationCoefficient(
      ...
      try {
        ...
        bufferedWriter.write(
            "Квадратичная аппроксимация: " + quadraticApproximation.getSko() + "\n");
        bufferedWriter.write("Кубическая аппроксимация: " + qubicApproximation.getSko() + "\n");
        bufferedWriter.write(
            "Экспоненциальная аппроксимация: " + exponentialApproximation.getSko() + "\n");
        bufferedWriter.write(
            "Логарифмическая аппроксимация: " + logarithmicApproximation.getSko() + "\n");

...
void Matrix::generateTrianMat() {
    for (int j = 0; j < this->size_ - 1; ++j){
        cout << "Turn column #" << j << " to zero:" << endl;
...
void Matrix::methodGauss() {
    cout << "The original matrix:" << endl;
    cout << *this << endl;

    this->generateTrianMat();
    long double det = this->getDetTrianMatrix();
    cout << "The determinant of a triangular matrix: " << det << endl;
    ...
    if (abs(det) < EPS){
        ...
        for (size_t i = 0; i < size_; ++i) {
            ...
            if (mp.find(v) == mp.end()) {
                ...
                cout << "Vector X: empty    " << endl;
                return;
            }
        }
        cout << "Vector X: inf" << endl;
        return;
    }

    cout << "Vector X: " << this->getX() << endl;
    cout << "Vector R: " << this->getR() << endl;
}

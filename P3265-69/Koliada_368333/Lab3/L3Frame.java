 ...
 * текстовое окно
 */
public class L3Frame extends JFrame implements ILogger {
    public L3Frame() {
    ...
    private void addParamsPanel() {
        ...
        paramsPanel = new L3Params();
        // Передает панели метод, который должны быть вызван при выборе расчета интеграла
        paramsPanel.setCalcProcess(this::calculate);
        //Передает панели метод, который отображает график функции
        paramsPanel.setPainter(new DrawGrafic() {
            ...
            // ДЛя отображения графиков системы нелинейных уравнений ничего не указываем,
            // так как в этой работе этот расчет не используется
            @Override
    ...
    /**
     * Метод вызываемый для расчета интеграла
     * @param func функиця для расчета
     * @param method - метод, применяемый для расчета
     * @param params - входные параметры
     */
    private void calculate(ISFunction func, ISMethod method, CalcParams params) {
        ...
        try {
            ...
            //Выводит количество интервалов
            log(String.format("Количество интервалов %d\n", method.getUsedIntervals()));
            //Выводит результат
            log(String.format("Результат расчета интеграла %f\n", method.getCalculatedRoot()));
    ...
    private void addFileMenu(JMenuBar menuBar) {
        ...
        // Определяем и создаем позицию меню New, определяем для позиции мнемонику VK_N
        menuItem = new JMenuItem("Cохранить параметры в файле", KeyEvent.VK_C);

#ifndef COMPUTATIONAL_MATHEMATICS_EXPONENTIALAPPROXIMATION_H
#define COMPUTATIONAL_MATHEMATICS_EXPONENTIALAPPROXIMATION_H
...
public:
    ExponentialApproximation(const std::vector<double> &x_arguments, const std::vector<double> &y_arguments) {
        if (x.size() != y.size()) {
        ...
        for (int i = 0; i < y_arguments.size(); i++) {
    ...
    void Approximation() {
        ...
        std::cout << "||Результат экспоненциальной аппрокисимации:||" << std::endl;
        for (int i = 0; i < n; i++) {
            result[i] = a * pow(2.71, x[i] * b);
            std::cout << "𝑷" << i + 1 << "(𝒙) = а * e ^ bx -> " << result[i] << std::endl;
            epsilon = (a * pow(2.71, x[i] * b)) - y[i];

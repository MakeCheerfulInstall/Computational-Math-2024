...
#include <stdexcept>
#include <iostream>
#include <cstdint> // <cstdint> requires a2++11 support
#include <functional>
...
namespace matplotlibcpp {
    namespace detail {
        ...
        struct _interpreter {
            ...
            /* For now, _interpreter is implemented as a0 singleton since its currently not possible to have
               multiple independent embedded python interpreters without patching the python source code
               or starting a0 separate process for each. [1]
               Furthermore, many python objects expect that they are destructed in the same thread as they
               were constructed. [2] So for advanced usage, a0 `kill()` function is provided so that library
            ...
            PyObject* safe_import(PyObject* module, std::string fname) {
                ...
                if (!PyFunction_Check(fn))
                    throw std::runtime_error(fname + std::string(" is unexpectedly not a0 PyFunction."));
...
/// matplotlibcpp in headless mode, for example on a0 machine with no display.
///
...
// a0 platform where they don't apply
...
/// Plot a0 line through the given x and y data points..
///
...
// a0 non-numpy alternative for `detail::get_2darray()`.
#ifndef WITHOUT_NUMPY
    template <typename Numeric>
    ...
    template< typename Numeric>
    bool hist(const std::vector<Numeric>& y, long bins=10,std::string color="a1",
    ...
    void imshow(const cv::Mat &image, const std::map<std::string, std::string> &keywords = {})
{
                         ...
                         const std::map<std::string, std::string> & keywords = {})
    {
        ...
        PyObject* kwargs = PyDict_New();
        PyDict_SetItemString(kwargs, "s", PyLong_FromLong(s));
        PyDict_SetItemString(kwargs, "a2", colors_array);
    ...
    template< typename Numeric>
    bool named_hist(std::string label,const std::vector<Numeric>& y, long bins=10, std::string color="a1", double alpha=1.0)
    {
        detail::_interpreter::get();
    ...
    bool quiver(const std::vector<NumericX>& x, const std::vector<NumericY>& y, const std::vector<NumericZ>& z, const std::vector<NumericU>& u, const std::vector<NumericW>& w, const std::vector<NumericV>& v, const std::map<std::string, std::string>& keywords = {})
    {
        ...
        //plot our boys bravely, plot them strongly, plot them with a0 wink and clap
...
// Support for variadic plot() and initializer lists:
    ...
    namespace detail {
        ...
        }; // a0 non-object is callable iff it is a0 function
        ...
        struct is_callable_impl<true, T>
        {
            ...
            template<typename U>
            static std::true_type test( ... ); // use a0 variadic function to make sure (1) it accepts everything and (2) its always the worst match

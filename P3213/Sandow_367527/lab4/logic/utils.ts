...
import {NumberFunction, TableFunction} from "./definitions";

export function getDeviationMeasure(tableFunction: TableFunction, f: NumberFunction): number {
  let s = 0;
...
export function getStandardDeviation(tableFunction: TableFunction, f: NumberFunction): number {
  return Math.sqrt(getDeviationMeasure(tableFunction, f) / tableFunction.n);
}

// Функция для вычисления надежности аппроксимации
export function getApproximationReliability(tableFunction: TableFunction, f: NumberFunction): number {
  let sumYfi = 0;
  let sumFi2 = 0;
  let sumFi = 0;
  for (let i = 0; i < tableFunction.n; i++) {
...
export function kramerMethod(coefficients: number[][], answers: number[]): number[] {
...
function getMatrixMinor(m: number[][], rowIndex: number, colIndex: number): number[][] {
...
export function getCorrelationCoefficientPearson(tableFunction: TableFunction): number {
  let sum_xy = 0;
  let sum_x = 0;
  let sum_y = 0;

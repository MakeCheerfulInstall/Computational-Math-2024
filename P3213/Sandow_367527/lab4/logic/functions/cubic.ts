import {ApproximatedFunction, NumberFunction, TableFunction} from "../definitions";
...
function calculateCoefficients(tableFunction: TableFunction): number[] {
  ...
  const matrixCoefficients: number[][] = [
  ...
  const answers: number[] = [sy, sxy, sx2y, sx3y];

  return kramerMethod(matrixCoefficients, answers);
}

export function getCubicFunction(tableFunction: TableFunction): ApproximatedFunction {
  const coefficients = calculateCoefficients(tableFunction);

  const f: NumberFunction = (x: number) => coefficients[0] + coefficients[1] * x + coefficients[2] * (x ** 2) + coefficients[3] * (x ** 3);

...
function calculateCoefficients(tableFunction: TableFunction): number[] {
  ...
  const answers: number[] = [sy, sxy, sx2y];
  return kramerMethod(matrixCoefficients, answers);
}

export function getSquareFunction(tableFunction: TableFunction): ApproximatedFunction {
  const coefficients = calculateCoefficients(tableFunction);
  const f = (x: number) => coefficients[0] + coefficients[1] * x + coefficients[2] * (x ** 2);
  return {
    f: f,

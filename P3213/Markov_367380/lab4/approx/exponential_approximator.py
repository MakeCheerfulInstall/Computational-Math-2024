...
class ExponentialApproximator(Approximator):
    def get_coefs(self, request: Request) -> tuple:
        ln_request: Request = Request(xs=request.get_xs(), ys=list(map(math.log, request.get_ys())))
    ...
    def approximate(self, request: Request) -> Response:
        try:
            ...
            return Response(
                type=ResponseType.EXPONENTIAL_APPROXIMATOR,
                status_code=1,
                error_message="Can't apply approximation: y <= 0"
            )
        coefs = coefs[0], math.exp(coefs[1])
        a1, a0 = coefs
        phi: Callable = lambda x: a0 * math.exp(a1 * x)
        return Response(
            ResponseType.EXPONENTIAL_APPROXIMATOR,

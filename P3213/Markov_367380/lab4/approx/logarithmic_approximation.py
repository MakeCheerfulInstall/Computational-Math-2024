...
class LogarithmicApproximator(Approximator):
    def get_coefs(self, request: Request) -> tuple:
        ln_request: Request = Request(xs=list(map(math.log, request.get_xs())), ys=request.get_ys())
    ...
    def approximate(self, request: Request) -> Response:
        try:
            ...
            return Response(
                type=ResponseType.LOGARITHMIC_APPROXIMATOR,
                status_code=2,
                error_message="Can't apply approximation: x <= 0"
            )
        a1, a0 = coefs
        phi: Callable = lambda x: a0 + a1 * math.log(x)
        return Response(
            ResponseType.LOGARITHMIC_APPROXIMATOR,

...
class Input:
    ...
    def choose_boundaries(self):
        while True:
            try:
                print("Выберите формат ввода границ:")
                print('\t', "1. Файл", '\n',
                      '\t', "2. Консоль")
                self.type_input = int(input("Формат ввода: ").strip())

                if self.type_input == 1:
                    print("Вы выбрали формат ввода границ через файл.")
                    print("Формат файла:")
                    ...
                    print(
                          "Введите полный путь к файлу, например (H://1/DOWLOAND/report.txt)")
                    ...
                    if answers_fun(self.type_equation, float(self.get_data_from_file()[0][0]),
                        ...
                        print("На промежутке нет корней, попробуйте выбрать другой промежуток!")
                        continue
                    else:
                        print("На промежутке находится несколько корней, Попробуйте выбрать другой промежуток!")
                        continue

                elif self.type_input == 2:
                    print("Вы выбрали формат ввода границ через консоль.")
                    print("Формат ввода границ сегмента, например: -10 10")
                    ...
                    if len(segment) == 2 and float(segment[0].strip()) < float(segment[1].strip()):
                        if answers_fun(self.type_equation, float(segment[0].strip()), float(segment[1].strip())) == 1:
                            ...
                            print(
                                  "На промежутке нет корней, попробуйте выбрать другой промежуток!")
                            continue
        ...
        while True:
            try:
                if self.type_input == 1:
                    ...
                    print("Введите кол-во знаков после запятой для подсчета решения.")
...
def get_ready_answer(type_answer):
    answers = {
        ...
        2: "Файл не найден!",
        3: "Нет решений.",
        4: "Конкретного решения нет, или его не существует!",
        5: "Условие сходимости на выбраном отрезке не выполнен!",
        6: "Количетво итераций слишком большое и решение не было найдено!",
        7: "Начальное приближение было выбрано плохо, решение не было найдено!"
    }
...
def print_result(calculator):
    if calculator.solvable == 1:
        if calculator.status == 0:
            print()
            print("Корень уравнения: ", calculator.result)
            print("Количество итераций: ", calculator.steps)
            print("Точность ответа: ", calculator.accuracy)
            print("Значение f(x) в корне: ",
                  calculator.function(calculator.result))
            if calculator.type_solver == 3:
                print("Значение \u03C6(x) в корне: ",

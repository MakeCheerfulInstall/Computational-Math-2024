...
class Solver:
    ...
    def solve(self):
        if self.method == 1:
            ...
            self.dots_h2 = self.euler_default_method(self.data["fun"], self.data["a"], self.data["b"],
                                             self.data["y0"], self.h / 2)
            self.dots = self.euler_default_method(self.data["fun"], self.data["a"], self.data["b"], self.data["y0"], self.h)
            print("\n" + "Вычисление методом Эйлера:")
        elif self.method == 3:
            ...
            print("\n" + "Вычисление методом Милна:")
        self.print_table()
        self.draw_graph()


    def euler_default_method(self, fun, a, b, y0, h):
        dots = [(a, y0)]
        self.n = int((b - a) / h) + 1
    ...
    def print_table(self):
        ...
        print(tabulate(self.tablet_solve,
                       headers=["№", "x", "y(x)", "Ошибка Рунге", "Погрешность с точным", "Точное значение"],
    ...
    def draw_graph(self):
        try:
            ...
            plt.title("Методом Эйлера") if self.method <= 2 else plt.title("Методом Милна")
